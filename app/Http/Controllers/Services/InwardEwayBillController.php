...
class InwardEwayBillController extends Controller
{
    ...
    public function generateEwayBill(Request $request, $order_id)
    {
        ...
        {
            return response()->json(['status' => 'error', 'message' => 'Order E-way Bill already Created', 'data' => []]);
        }
        ...
        if ($order) {
            ...
            if (!$isRecordCreated)
                return response()->json(['status' => 'error', 'message' => 'E-waybill created but failed to save response', 'data' => []]);

            return response()->json(['status' => 'success', 'message' => 'E-wayBill has been created by :' . ($response['display_message']) ?? '', 'data' => []]);
        }
