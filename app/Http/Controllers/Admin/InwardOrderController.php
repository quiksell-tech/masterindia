...
class InwardOrderController extends Controller
{
    ...
    {
        $orders = MiInwardOrder::with('items')->latest()->paginate(10);
        return view('inwardorders.index', compact('orders'));
    }
    ...
    public function create()
    {
        ...
        $latestDate = MiInwardOrder::max('order_invoice_date');
        ...
        return view('inwardorders.create', compact('transporters', 'billFromAddress', 'billFromParty', 'today', 'latestDate', 'lastDateOfMonth', 'defaultDate'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (strtolower($request->supply_type) === 'inward') {
            ...
            $rules['order_invoice_number'] = [
                ...
                'string',
                'max:50',
                Rule::unique('mi_inward_order', 'order_invoice_number'),
            ];
        }
        ...
        $order = MiInwardOrder::create($order);


        return redirect()
            ->route('inward.orders.edit', $order->order_id)
            ->with('success', 'Order created successfully. You can continue editing.');
    }

    public function edit(MiInwardOrder $order)
    {
        $items = MiInwardOrderItem::where('order_id', $order->order_id)->get();
        ...
        return view('inwardorders.edit', compact('order', 'order', 'items', 'allItems', 'transporters', 'latestDate', 'today', 'latestDate', 'lastDateOfMonth', 'defaultDate', 'orderInvoiceDate', 'allowedActions'));
    }

    public function invoiceData(MiInwardOrder $order)
    {
        $order->load([
    ...
    public function update(Request $request, MiInwardOrder $order)
    {
