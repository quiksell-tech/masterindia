...
@section('content')
    ...
    <form method="POST" action="{{ route('inward.orders.update', $order->order_id) }}">
        @csrf
    ...
    <form method="POST" action="{{ route('inward.orders.items.save', $order->order_id) }}">
        @csrf
    ...
    <script>
        ...
        $('#ewayBillForm').submit(function (e) {
            ...
            let url = actionType === 'cancel'
                ? `/api/inward-eway-bill/${orderId}/cancel`
                : `/api/inward-eway-bill/${orderId}/update`;
...
@endforeach
        ...
        function isItemAlreadySelected(itemId, currentRow) {
            let exists = false;

            document.querySelectorAll('.item-select').forEach(select => {
                if (
                    select !== currentRow.querySelector('.item-select') &&
                    select.value === itemId
                ) {
                    exists = true;
                }
        ...
        document.addEventListener('input', function (e) {
            ...
            if (
                ...
                if (selectedItemId && isItemAlreadySelected(selectedItemId, row)) {
                    alert('This item is already added.');
                    itemSelect.value = '';
                    row.querySelector('.item_code').value = '';
                    row.querySelector('.hsn_code').value = '';
                    row.querySelector('.taxable_amount').value = '';
                    row.querySelector('.after_tax_value').value = '';
                    return;
                }
    ...
    <script>
        flatpickr("#transportation_date", {
            ...
            {
                $.ajax({
                    url: "{{ url('api/inward-eway-bill') }}/" + orderId + "/generate",
                    type: "POST",
    ...
    <script>
        ...
        $('#cancelEinvoiceBtn').on('click', function () {
            ...
            let url = `/api/inward-einvoce/${orderId}/cancel`;
            let btn = $(this);
        ...
        {
            $.ajax({
                url: "{{ url('api/inward-einvoce') }}/" + orderId + "/generate",
                type: "POST",
